...
async function generateImagesWithOpenAI(input: GenerateComicPanelInput): Promise<GenerateComicPanelOutput> {
  try {
    ...
    const selectedOpenAIModel = 'gpt-image-1';
    ...
    for (const promptItem of input.promptsWithContext) {
      ...
      // Support both base64 and URL responses from OpenAI
      const imageData = imageResponse.data?.[0];
      let generatedImageDataUri: string | null = null;
      if (imageData?.b64_json) {
        generatedImageDataUri = `data:image/png;base64,${imageData.b64_json}`;
      } else if (imageData?.url) {
        generatedImageDataUri = await blobUrlToDataUri(imageData.url);
      }
