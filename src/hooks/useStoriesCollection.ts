...
import { uploadToVercelBlob, deleteImageFromBlob } from '@/lib/blob-storage';
...
export function useStoriesCollection() {
  ...
  const deletePanelImagesFromBlob = useCallback(async (panels: ComicPanelData[]) => {
    const urls: string[] = [];
    ...
    await Promise.allSettled(urls.map((u) => deleteImageFromBlob(u)));
  }, [isBlobUrl]);
  ...
  // Update panels for the active story (moved earlier to avoid TDZ for downstream callbacks)
  ...
  const deleteStory = useCallback(async (storyId: string): Promise<boolean> => {
    ...
    // Attempt to delete all Blob images associated with this story
    try {
      const storedPanelsJson = safeLocalStorage.getItem(`${LOCAL_STORAGE_PANELS_PREFIX}${storyId}`);
      const panelsForDeletion: ComicPanelData[] = storedPanelsJson
        ? JSON.parse(storedPanelsJson)
        : (storiesCollection.activeStoryId === storyId ? activeStoryPanels : []);
      await deletePanelImagesFromBlob(panelsForDeletion);
    } catch (err) {
      console.warn('Failed during blob cleanup for deleted story:', err);
    }
    ...
    return true;
  }, [storiesCollection, activeStoryPanels, saveStoriesCollection, saveReactFlowData, deletePanelImagesFromBlob, toast]);

  // Delete a single panel (and its descendants) from the active story and clean blob images
  const deletePanel = useCallback(async (panelId: string): Promise<boolean> => {
    if (!storiesCollection.activeStoryId) {
      toast({ title: 'Delete Error', description: 'No active story selected.', variant: 'destructive' });
      return false;
    }

    // Build a map for quick lookup
    const panelMap = new Map(activeStoryPanels.map(p => [p.id, p]));
    if (!panelMap.has(panelId)) {
      toast({ title: 'Delete Error', description: 'Panel not found.', variant: 'destructive' });
      return false;
    }

    // Collect all panels to remove (panel + descendants)
    const toRemoveIds = new Set<string>();
    const stack = [panelId];
    while (stack.length) {
      const id = stack.pop()!;
      if (toRemoveIds.has(id)) continue;
      toRemoveIds.add(id);
      const p = panelMap.get(id);
      if (p && Array.isArray(p.childrenIds)) {
        for (const cid of p.childrenIds) stack.push(cid);
      }
    }

    const panelsToDelete = activeStoryPanels.filter(p => toRemoveIds.has(p.id));
    // Cleanup blob images
    try {
      await deletePanelImagesFromBlob(panelsToDelete);
    } catch (err) {
      console.warn('Failed during blob cleanup for deleted panel(s):', err);
    }

    // Produce updated panel list without removed ones and update parents' childrenIds
    const remainingPanels = activeStoryPanels
      .filter(p => !toRemoveIds.has(p.id))
      .map(p => ({
        ...p,
        childrenIds: p.childrenIds.filter(id => !toRemoveIds.has(id)),
      }));

    await updateActiveStoryPanels(remainingPanels);
    toast({ title: 'Panel Deleted', description: 'The panel has been removed.' });
    return true;
  }, [storiesCollection.activeStoryId, activeStoryPanels, updateActiveStoryPanels, deletePanelImagesFromBlob, toast]);
