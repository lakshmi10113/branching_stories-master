...
export default function Main() {
  ...
  const handleUploadInitialPanel = useCallback(async () => {
    // Ensure a story exists before opening upload
    if (!activeStory) {
      await createNewStory("New Story");
    }
    setIsUploadDialogOpen(true);
    navigateToStoryEditor();
  }, [activeStory, createNewStory, navigateToStoryEditor]);
  ...
  const addPanel = useCallback(async ({ imageUrls, parentId, promptsUsed, userDescription }: {
    imageUrls: string[];
    parentId: string | null;
    promptsUsed?: string[];
    userDescription?: string;
  }) => {
    // Ensure there is an active story before adding
    if (!activeStory) {
      await createNewStory("New Story");
    ...
    await updateActiveStoryPanels(updatedPanels);
    return newPanelId;
  }, [panels, activeStory, createNewStory, updateActiveStoryPanels]);
